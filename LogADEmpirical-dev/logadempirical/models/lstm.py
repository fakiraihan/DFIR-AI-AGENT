...
if __name__ == '__main__':
    # Test code for DeepLog and LogAnomaly
    deeplog = DeepLog(128, 2, 100, 128)
    loganomaly = LogAnomaly(128, 2, 100, 300)
    
    sem_inp = torch.rand(64, 100, 300)
    quan_inp = torch.rand(64, 100, 1)
    seq_inp = torch.randint(100, (64, 100))
    pred_labels = torch.randint(100, (64,))
    
    batch_deeplog = {'sequential': seq_inp, 'label': pred_labels}
    batch_loganomaly = {'semantic': sem_inp, 'quantitative': quan_inp, 'label': pred_labels}
    
    out = deeplog(batch_deeplog)
    print("DeepLog: ", out.loss)
    
    out = loganomaly(batch_loganomaly)
