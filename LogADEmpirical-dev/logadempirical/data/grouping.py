...
def fixed_window(raw_data, window_size, step_size):
    """
    split logs into sliding windows/session
    :param raw_data: dataframe columns=[label, eventid, eventtemplate, content] or [timestamp, label, eventid, eventtemplate, content]
    ...
    # Check if timestamp is included (5 columns) or not (4 columns)
    if raw_data.shape[1] == 5:
        # Has timestamp: [timestamp, label, eventid, eventtemplate, content]
        time_data = raw_data.iloc[:, 0]
        label_data = raw_data.iloc[:, 1]
        logkey_data = raw_data.iloc[:, 2]
        log_template_data = raw_data.iloc[:, 3]
        content_data = raw_data.iloc[:, 4]
    else:
        # No timestamp: [label, eventid, eventtemplate, content]
        label_data = raw_data.iloc[:, 0]
        logkey_data = raw_data.iloc[:, 1]
        log_template_data = raw_data.iloc[:, 2]
