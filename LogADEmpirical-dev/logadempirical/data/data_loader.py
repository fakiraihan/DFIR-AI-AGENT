...
def process_dataset(logger: Logger,
    ...
    if grouping == "sliding":
        ...
        if not is_chronological:
            logger.info(f"DEBUG: df shape before sliding: {df.shape}")
            window_df = sliding(
                df[["Label", "EventId", "EventTemplate", "Content"]],
                window_size=window_size,
                step_size=step_size
            )
            logger.info(f"DEBUG: Generated {len(window_df)} windows from sliding window")
            ...
            logger.info(f"DEBUG: Train windows: {len(train_window)}, Test windows: {len(test_window)}")
        else:
